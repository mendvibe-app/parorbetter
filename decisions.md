...
- **Putt camera zoom-in (short putts + roll tighten):** old `half_span` floor (34) + zoom cap (7.5) together clamped every putt under ~70 ft to one flat zoom — short and 40 ft putts looked identical. New floor 12 / pad 6 / cap 42 so `<~10 ft` reads clearly tighter than 20-40 ft, and lag putts still zoom out. Roll also gets a TV-style punch-in: inside `PUTT_TIGHTEN_RADIUS` (~15 ft) from the cup, zoom lerp jumps to the same 0.35 full-swing "in" speed and the look target biases 0.5 (was flat 0.35) toward the cup — gated on distance since putts have no air time. Post-settle framing already recomputes from the ball's final resting spot, so a close putt naturally holds its tight shot through the result without extra state.
